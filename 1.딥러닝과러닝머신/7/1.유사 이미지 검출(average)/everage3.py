...
def average_hash(fname, size = 16):
    ...
    if not os.path.exists(cache_file): #해시 생성하기
        ...
        img = img.convert('L').resize((size, size), Image.ANTIALIAS) #그레이스케일로 변환하기
        pixels = np.array(img.getdata()).reshape((size,size)) #Numpy 배열로 변환하기
        avg = pixels.mean() #평균 구하기
        px = 1*(pixels > avg) #평균보다 크면 1, 작으면 0으로 변환하기
        np.savetxt(cache_file, px, fmt="%.0f", delimiter=",")
    else: #캐시돼 있다면 읽지 않기
        np.loadtxt(cache_file, delimiter=",")
    return px
