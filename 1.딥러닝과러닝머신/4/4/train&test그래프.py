...
for i in range(0, len(train_label)):
    label = train_label[i]            #i=0 label은 a 를 갖고
    data = train_data[i]              #data는 a의 횟수 만큼을 가질테고
    if not (label in graph_dict):     #당연히 graph_dict 안에는 lable=a 가없으니 참이고,
        graph_dict[label] = data      #그 결과 graph_dict[0]=a의 횟수인 data를 가짐.
